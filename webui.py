...
def render_stats(stats, routes, node_id, sub_base, blocked_devices=None, csrf=""):
    ...
    for rid, st in items:
        ...
        for d in st.get("devices", []):
            ...
            rows.append(
                f'<tr><td>{esc(d.get("personal_name") or "—")}</td><td style="max-width:220px;overflow-wrap:anywhere">{esc(d.get("personal_contact") or "—")}</td><td><span class="mono">{label[:20]}</span>{badge}</td>'
        ...
        blocks.append(
            ...
            f'<table class="tbl" style="margin-top:8px"><thead><tr><th>Имя</th><th>Telegram / WhatsApp</th><th>HWID</th><th>Модель</th><th>Клиент</th><th>IP</th><th>Запр.</th><th>Узлы</th><th>Активность</th><th>Доступ</th></tr></thead>'
